...
resDir = 'grappa_flow_res_PSIR_correctedAIF';

resDir = 'grappa_flow_res_BTEX20';

resDir = 'grappa_flow_res_BTEX20_2';
resDir = 'grappa_flow_res_BTEX20_5';
resDir = 'grappa_flow_res_BTEX20_6';
...
for ii=1:num 
    ...
    fa_pde = fa_pde/100;
    aif_rest = analyze75read(fullfile(restDir, 'DebugOutput', 'aif_cin_echo0_all_signal_baseline_corrected.hdr'));
    ...
    fb_pde = fb_pde/100;
    aif_stress = analyze75read(fullfile(stressDir, 'DebugOutput', 'aif_cin_echo0_all_signal_baseline_corrected.hdr'));
...
size(ResultRecordRest)

ages = []
for ii=1:size(ResultRecordRest, 1)   
    ages = [ages ResultRecordRest{ii, 13}];    
end
...
male = 0;
for ii=1:3:size(ResultRecordRest, 1)   
    if(strcmp(ResultRecordRest{ii, 12}, 'M')==1)
        male = male + 1;
    end
end
male
...
aif_peak = [];
aif_peak_without_R2Star = [];
aif_peak_2ml = [];
aif_peak_2ml_without_R2Star = [];

for ii=1:3:size(ResultRecordRest, 1) 
    
    if( ~isempty(strfind(ResultRecordRest{ii, 2}, '2ml')))
        aif_peak_2ml = [aif_peak_2ml ResultRecordRest{ii, 7}];
        aif_peak_2ml_without_R2Star = [aif_peak_2ml_without_R2Star ResultRecordRest{ii, 15}];
    else
        aif_peak = [aif_peak ResultRecordRest{ii, 7}];
        aif_peak_without_R2Star = [aif_peak_without_R2Star ResultRecordRest{ii, 15}];
    end
end
...
mean(aif_peak_without_R2Star)
std(aif_peak_without_R2Star)

[h, p] = ttest(aif_peak, aif_peak_without_R2Star)

mean(aif_peak_2ml)
std(aif_peak_2ml)

mean(aif_peak_2ml_without_R2Star)
std(aif_peak_2ml_without_R2Star)

[h, p] = ttest(aif_peak_2ml, aif_peak_2ml_without_R2Star)

aif_width = [];
aif_width_2ml = [];
for ii=1:3:size(ResultRecordRest, 1)   
    if( ~isempty(strfind(ResultRecordRest{ii, 2}, '2ml')))
        aif_width_2ml = [aif_width_2ml ResultRecordRest{ii, 11}-ResultRecordRest{ii, 9}];
    else
        aif_width = [aif_width ResultRecordRest{ii, 11}-ResultRecordRest{ii, 9}];
    end
end
...
mean(aif_width_2ml*0.5)
std(aif_width_2ml*0.5)

%% EF vs. aif width plot

EF_aif_width = [];
EF_aif_width_cases = [];
EF_aif_width_2ml = [];
EF_aif_width_cases_2ml = [];
for ii=1:3:size(ResultRecordRest, 1)      
    if(ResultRecordRest{ii, 14}>0)
        if( ~isempty(strfind(ResultRecordRest{ii, 2}, '2ml')))
            EF_aif_width_cases_2ml = [EF_aif_width_cases_2ml; {ResultRecordRest{ii, 2}}];
            EF_aif_width_2ml = [EF_aif_width_2ml; ResultRecordRest{ii, 14} ResultRecordRest{ii, 11}-ResultRecordRest{ii, 9}]; 
        else
            EF_aif_width_cases = [EF_aif_width_cases; {ResultRecordRest{ii, 2}}];
            EF_aif_width = [EF_aif_width; ResultRecordRest{ii, 14} ResultRecordRest{ii, 11}-ResultRecordRest{ii, 9}]; 
        end
    end
end
...
figure
hold on
plot(EF_aif_width(:, 1), EF_aif_width(:, 2)*0.5, '.');
hold off
xlabel('LVEF (%)');
ylabel('AIF First Pass Width (second)');
box on

save aifs aif_peak aif_width EF_aif_width EF_aif_width_2ml

cd e:\gtuser\gt_windows_setup\ut\DualBolus\FLASH
a1 = load('aifs.mat');

cd e:\gtuser\gt_windows_setup\ut\DualBolus\SSFP
a2 = load('aifs.mat');

cd E:\gtuser\gt_windows_setup\ut\DualBolus\SUB_Perf\NewSeq
a3 = load('aifs.mat');

figure
hold on
plot(a3.EF_aif_width(:, 1), a3.EF_aif_width(:, 2)*0.5, 'b.');
plot(a3.EF_aif_width_2ml(:, 1), a3.EF_aif_width_2ml(:, 2)*0.5, 'b+');
plot(a2.EF_aif_width(:, 1), a2.EF_aif_width(:, 2)*0.5, 'r.');
plot(a1.EF_aif_width(:, 1), a1.EF_aif_width(:, 2)*0.5, 'kx');
hold off
legend('1.5T, 4ml/s, 0.05mmol/kg', '1.5T, 2ml/s, 0.05mmol/kg', '3T, 4ml/s, 0.05mmol/kg', '3T, 4ml/s, 0.075mmol/kg');
xlabel('LVEF (%)');
ylabel('AIF First Pass Width (second)');
box on
...
ind = find(rest_pde_pt>0.05 & rest_pde_pt<2 & rest_pt>0.05 & rest_pt<2);
ind2 = find(stress_pde_pt>0.05 & stress_pt>0.05 & stress_pde_pt<8 & stress_pt<5);
   
figure
hold on
plot(stress_pde_pt(ind2), stress_pt(ind2), 'r.');
plot(rest_pde_pt(ind), rest_pt(ind), '.');
plot([0 8], [0 8], 'k-');
hold off
legend('stress', 'rest')
xlabel('PDE Flow (ml/min/g)');
ylabel('Ki (ml/min/g)');
box on
...
figure
hold on
plot(stress_t1_blood(:, 1), rest_t1_blood(:, 1), 'r+');
plot(stress_t1_myo(:, 1), rest_t1_myo(:, 1), 'b.');
hold off
legend('blood', 'myo');
xlabel('stress')
ylabel('rest')
...
mean(stress_t1_blood(:, 1))
std(stress_t1_blood(:, 1))
mean(stress_t1_myo(:, 1))
std(stress_t1_myo(:, 1))

mean(rest_t1_blood(:, 1))
std(rest_t1_blood(:, 1))
mean(rest_t1_myo(:, 1))
std(rest_t1_myo(:, 1))

% stress-rest time diff
mean(rest_t1_blood(:, 2))
std(rest_t1_blood(:, 2))

y = zeros(2, 2);
err = zeros(2, 2, 2);
y(1, 1) = mean(stress_t1_blood(:, 1));
y(1, 2) = mean(rest_t1_blood(:, 1));
y(2, 1) = mean(stress_t1_myo(:, 1));
y(2, 2) = mean(rest_t1_myo(:, 1));

err(1, 1, 1) = max(stress_t1_blood(:, 1));
err(1, 1, 2) = min(stress_t1_blood(:, 1));
err(1, 2, 1) = max(rest_t1_blood(:, 1));
err(1, 2, 2) = min(rest_t1_blood(:, 1));

err(2, 1, 1) = max(stress_t1_myo(:, 1));
err(2, 1, 2) = min(stress_t1_myo(:, 1));
err(2, 2, 1) = max(rest_t1_myo(:, 1));
err(2, 2, 2) = min(rest_t1_myo(:, 1));
barwitherr(err, y);
set(gca, 'XTickLabel',{'Blood','Myo'})
legend('Pre-stress','Pre-rest')
ylabel('T1 (ms)')


%% save results

save flow_map_records Ki_map_rest_normal Ki_map_stress_normal Flow_map_rest_normal Flow_map_stress_normal Ki_map_rest_sick Ki_map_stress_sick Flow_map_rest_sick Flow_map_stress_sick

figure; imagescn(Flow_map_stress_sick, [0 8]); PerfColorMap;
figure; imagescn(Flow_map_stress_normal, [0 10]); PerfColorMap;
figure; imagescn(Ki_map_stress_normal, [0 8]); PerfColorMap;
...
ind = 1;
clear restPerf_ill stressPerf_ill restGd_ill stressGd_ill restPerf_flow_ill stressPerf_flow_ill
N = size(ResultRecordRest, 1)
for n=1:N
    normal = 0;
    for ii=1:numNorm
        if(strcmp(normalCases{ii}, ResultRecordRest{n, 2})==1)
            normal = 1;
            break;
        end
    end
    ...
    if(~normal)
        restPerf_ill(ind) = ResultRecordRest{n, 5};
        stressPerf_ill(ind) = ResultRecordStress{n, 5};
        restPerf_flow_ill(ind) = ResultRecordRest{n, 6};
        stressPerf_flow_ill(ind) = ResultRecordStress{n, 6};
        restGd_ill(ind) = ResultRecordRest{n, 7};
        stressGd_ill(ind) = ResultRecordStress{n, 7};
        ind = ind + 1;
    end
end
...
mean(restPerf_flow_ill)
std(restPerf_flow_ill)

mean(stressPerf_flow_ill)
std(stressPerf_flow_ill)

% ------------------------

ind = 1;
clear restPerf_normal stressPerf_normal restGd_normal stressGd_normal restPerf_flow_normal stressPerf_flow_normal
for n=1:N
    normal = 0;
    for ii=1:numNorm
        if(strcmp(normalCases{ii}, ResultRecordRest{n, 2})==1)
    ...
    if(normal)
        restPerf_normal(ind) = ResultRecordRest{n, 5};
        stressPerf_normal(ind) = ResultRecordStress{n, 5};
        restPerf_flow_normal(ind) = ResultRecordRest{n, 6};
        stressPerf_flow_normal(ind) = ResultRecordStress{n, 6};
        restGd_normal(ind) = ResultRecordRest{n, 7};
        stressGd_normal(ind) = ResultRecordStress{n, 7};
        ind = ind + 1;
    end
end

size(restPerf_normal)

mean(restPerf_normal)
std(restPerf_normal)

mean(stressPerf_normal)
std(stressPerf_normal)

mean(restPerf_flow_normal)
std(restPerf_flow_normal)

mean(stressPerf_flow_normal)
std(stressPerf_flow_normal)

y = zeros(2, 2);
err = zeros(2, 2);
y(1, 1) = mean(restPerf_ill);
y(1, 2) = mean(restPerf_flow_ill);
y(2, 1) = mean(restPerf_normal);
y(2, 2) = mean(restPerf_flow_normal);

err(1, 1) = std(restPerf_ill(:, 1));
err(1, 2) = std(restPerf_flow_ill(:, 1));
err(2, 1) = std(restPerf_normal(:, 1));
err(2, 2) = std(restPerf_flow_normal(:, 1));
...
figure
hold on
scatter(ones(N,1), restPerf, 'b+');
scatter(ones(N,1)+1, stressPerf, 'bo');
legend('rest', 'stress')
line([ones(N,1) ones(N,1)+1]', [restPerf(:) stressPerf(:)]', 'LineStyle', '--', 'Color', 'b');
hold off
xlim([0 3]) 
set(gca,'XTick',[0 1 2 3])
set(gca,'XTickLabel',{'', 'rest', 'stress', ''})
ylabel('flow (ml/min/g)');
box on
title(home);

figure
hold on
scatter(ones(N,1), restGd, 'b+');
scatter(ones(N,1)+1, stressGd, 'bo');
legend('rest', 'stress')
line([ones(N,1) ones(N,1)+1]', [restGd(:) stressGd(:)]', 'LineStyle', '--', 'Color', 'b');
hold off
xlim([0 3]) 
set(gca,'XTick',[0 1 2 3])
set(gca,'XTickLabel',{'', 'rest', 'stress', ''})
ylabel('AIF peak Gd (mmol/ml)');
box on
title(home);
...
scatter(ones(N,1), restPerf, 'b+');
scatter(ones(N,1)+1, stressPerf, 'bo');
scatter(ones(numel(indNew),1), restPerf(indNew), 'r+');
scatter(ones(numel(indNew),1)+1, stressPerf(indNew), 'ro');
legend('rest', 'stress', 'rest new seq', 'stress new seq')
line([ones(N,1) ones(N,1)+1]', [restPerf(:) stressPerf(:)]', 'LineStyle', '--', 'Color', 'b');
line([ones(numel(indNew),1) ones(numel(indNew),1)+1]', [restPerf(indNew(:)); stressPerf(indNew(:))], 'LineStyle', '--', 'Color', 'r');
hold off
xlim([0 3]) 
set(gca,'XTick',[0 1 2 3])
set(gca,'XTickLabel',{'', 'rest', 'stress', ''})
...
scatter(ones(N,1), restGd, 'b+');
scatter(ones(N,1)+1, stressGd, 'bo');
scatter(ones(numel(indNew),1), restGd(indNew), 'r+');
scatter(ones(numel(indNew),1)+1, stressGd(indNew), 'ro');
legend('rest', 'stress', 'rest new seq', 'stress new seq')
line([ones(N,1) ones(N,1)+1]', [restGd(:) stressGd(:)]', 'LineStyle', '--', 'Color', 'b');
line([ones(numel(indNew),1) ones(numel(indNew),1)+1]', [restGd(indNew(:)); stressGd(indNew(:))], 'LineStyle', '--', 'Color', 'r');
hold off
xlim([0 3]) 
set(gca,'XTick',[0 1 2 3])
set(gca,'XTickLabel',{'', 'rest', 'stress', ''})
