function data = DicomConverterWithPixelSize(home, dstDir, PixelSpacing)
    ...
    for s = 1:fileNum
        if ( D(s).isdir == 0 )
            ...
            if ( isdicom(currentFileName) )
                ...
                if ( isfield(cinfo, 'InstanceNumber') )
                    info.AcquisitionNumber = cinfo.InstanceNumber;                
                end

                if ( ~isfield(cinfo, 'PixelSpacing') )
                    cinfo.PixelSpacing(1) = PixelSpacing(1);
                    cinfo.PixelSpacing(2) = PixelSpacing(2);
                end
                
                if ( ~isfield(cinfo, 'SliceThickness') )
                    cinfo.SliceThickness = PixelSpacing(3);
                end
