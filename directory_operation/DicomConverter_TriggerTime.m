function headers = DicomConverter_TriggerTime(home, dstDir, suffix)
    ...
    for s = 1:fileNum
        if ( D(s).isdir == 0 )
            ...
            if ( isdicom(currentFileName) )
                ...
                if ( isfield(cinfo, 'TriggerTime') )
                    info.TriggerTime = cinfo.TriggerTime;                
                end
...
for pp = 1:numPatients
    ...
    for serials = 1:numOfSerials
        ...
        for sliceP = 1:numOfSlicePositions
            ...
            for k=1:numFile
                AcquisitionNumber(k) = cFileInfo_SlicePostions(k).TriggerTime;
            end
            ...
            if ( isempty(suffix) )
                Prefix = [num2str(cSerials) '_' num2str(cSlicePosition)];
            else
                Prefix = [num2str(cSerials) '_' num2str(cSlicePosition) '_' suffix];
            end
