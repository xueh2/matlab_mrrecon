...
function VesselExtractionRun(currentDir, tofResampled, extractionResults)
...
zbsflag = 0;
coefficientflag = 0;
coefficientblurredflag = 0;

zbs_leftup = [];
zbs_rightdown = [];

[vasculature, coefficient, vasculatureDetected] = RidgeDetection5(data, header, zbs_leftup, zbs_rightdown,...
    seedfactor, samplestepnew, parameters, radiusparameters, factor, suffix,...
    doradius, zbsflag, coefficientflag, coefficientblurredflag, background, t);

% save vasculature into the extractionResults directory...
vesselfile = fullfile(currentDir, extractionResults, 'vasculature.mat');
