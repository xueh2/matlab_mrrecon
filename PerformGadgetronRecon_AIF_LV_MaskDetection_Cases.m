...
for ii=startN:endN
    ...
    try
        ...
        case_dir = perf_cases{ii,2};
        
        % [dataDir, resDir, study_dates] = find_SavedIsmrmrd_data_result_dir(case_dir)
        
        % stress
        stress_dir = fullfile(resDir, study_dates, case_dir)
        ...
        [L_gt, numroi_gt] = bwlabel(mask_stress_gt, 4);
        C_gt = regionprops(L_gt, 'Centroid');
        R_gt = regionprops(L_gt, 'EquivDiameter');
...
%         h = figure('Name',case_dir,'NumberTitle','off'); imagescn(cat(3, aif_slc, aif_slc_mask, aif_slc), [0.1*minI 0.9*maxI], [1 3], [10]);
%         posh = get(h, 'Position');
%         set(h, 'Position', [posh(1) 6 posh(3) posh(4)]);
%         hold on
%         z = R.EquivDiameter/2*exp((0:.1:2*pi)*i);
%         plot(real(z)+C.Centroid(1), imag(z)+C.Centroid(2), 'r-', 'LineWidth', 2);
%         hold off
%         saveas(h, fullfile(fig_Dir, case_dir), 'fig');
%         ff = getframe(h);
%         [X, map] = frame2im(ff);
%         imwrite(X, fullfile(fig_Dir, [case_dir '.bmp']), 'BMP');
%         
        h = figure('Name', [case_dir '_GT'],'NumberTitle','off'); imagescn(cat(3, aif_slc, aif_slc_mask, aif_slc_mask_gt, aif_slc), [0.1*minI 0.9*maxI], [1 4], [10]);
        posh = get(h, 'Position');
        set(h, 'Position', [posh(1) 6 posh(3) posh(4)]);
        hold on
        z = R_gt.EquivDiameter/2*exp((0:.1:2*pi)*i);
        plot(real(z)+C_gt.Centroid(1), imag(z)+C_gt.Centroid(2), 'r-', 'LineWidth', 2);
        hold off
        saveas(h, fullfile(fig_Dir, [case_dir '_GT']), 'fig');
        ff = getframe(h);
        [X, map] = frame2im(ff);
        imwrite(X, fullfile(fig_Dir, [case_dir '_GT.bmp']), 'BMP');
...
%         
%         t = 0.5 * [0:N-1];
%         h2 = figure('Name',case_dir,'NumberTitle','off', 'Position', [40 300 1500 700]);
%         hold on
...
%         grid on
%         
%         saveas(h2, fullfile(fig_Dir, [case_dir '_eigenMean']), 'fig');
%         ff = getframe(h2);
%         [X, map] = frame2im(ff);
%         imwrite(X, fullfile(fig_Dir, [case_dir '_eigenMean.bmp']), 'BMP');       
        ...
        if(size(perf_cases, 2)>=3)
            ...
            case_dir = perf_cases{ii, 3};
            
            [configName, scannerID, patientID, studyID, measurementID, study_dates, study_year, study_month, study_day, study_time_rest] = parseSavedISMRMRD(case_dir);
            ...
            tic
            mask_rest_gt = Matlab_gt_perfusion_flow_aif_mask(single(aif_pd_rest), single(aif_moco_rest), upslope_thres, 0.85, auc_thres, area_thres, max_duration, ro_boundary_ratio, e1_boundary_ratio, 'kmeans', []);
            disp(['matlab mex AIF masking - ' num2str(toc)]);
            
            ind = find(mask>0);
            ind_RV = find(mask_RV_rest>0);
            ind_gt = find(mask_rest_gt>0);
            ...
            [L_gt, numroi_gt] = bwlabel(mask_rest_gt, 4);
            C_gt = regionprops(L_gt, 'Centroid');
            R_gt = regionprops(L_gt, 'EquivDiameter');
...
%             imwrite(X, fullfile(fig_Dir, [perf_cases{ii,3} '.bmp']), 'BMP');
            ...
            fig_name = [perf_cases{ii,3} '_GT'];
            h = figure('Name', fig_name,'NumberTitle','off'); imagescn(cat(3, aif_slc, aif_slc_mask, aif_slc_mask_gt, aif_slc), [], [1 4], [10]);
            posh = get(h, 'Position');
            set(h, 'Position', [posh(1) 6 posh(3) posh(4)]);
            hold on
            z = R_gt.EquivDiameter/2*exp((0:.1:2*pi)*i);
            plot(real(z)+C_gt.Centroid(1), imag(z)+C_gt.Centroid(2), 'r-', 'LineWidth', 2);
            hold off            
            saveas(h, fullfile(fig_Dir, fig_name), 'fig');
            ff = getframe(h);
            [X, map] = frame2im(ff);
            imwrite(X, fullfile(fig_Dir, [fig_name '.bmp']), 'BMP');
