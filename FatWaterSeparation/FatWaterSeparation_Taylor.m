...
[points, edges] = lattice(Nfe,Npe,3);
...
for iter=1:params.MAXIT
    ...
    for pe=1:Npe
        for fe=1:Nfe               
            ...
            rouq_pixel = Water(fe,pe)+Fat(fe,pe).*phi(:,2);
            rouq(fe, pe, :) = reshape(rouq_pixel, [1 1 numOfEcho]);
            dq_pixel = s - exp(-R2SMap(fe,pe).*TEs).*exp(i*2*pi*B0Map(fe,pe).*TEs).*( 1 - B0Map(fe,pe)*i*2*pi*TEs ).*rouq_pixel;
            dq(fe, pe, :) = reshape(dq_pixel, [1 1 numOfEcho]);
        end
    end
    
    S = zeros(Nfe, Npe);
    T = sparse(1:Nfe*Npe, 1:Nfe*Npe, 1, Nfe*Npe, Nfe*Npe);
    for pe=1:Npe
        for fe=1:Nfe               
            
            rouq_pixel = rouq(fe,pe,:);
            rouq_pixel = reshape(rouq_pixel, [numOfEcho 1]);
            
            dq_pixel = dq(fe,pe,:);
            dq_pixel = reshape(dq_pixel, [numOfEcho 1]);
            
            commonTerm = exp(-R2SMap(fe,pe)*TEs).*(i*2*pi*TEs.*exp(i*2*pi*B0Map(fe,pe).*TEs)).*rouq_pixel;
            
            S_pixel = -2*commonTerm.*dq_pixel;            
            S(fe, pe) = sum(S_pixel);
            
            T_pixel = commonTerm;
            T(fe+(pe-1)*Nfe, fe+(pe-1)*Nfe) = 2*sum(T_pixel.*T_pixel);
        end
    end
        
    % 2) compute every item of T, column-wise storage
