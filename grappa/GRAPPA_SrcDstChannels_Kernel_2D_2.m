...
function Coef = GRAPPA_SrcDstChannels_Kernel_2D_2(kspace, acs, option, thresReg)
...
kPE = numel(option.KernelPattern);
...
Coef2 = zeros(KernelSize(1), KernelSize(2), srcCha, dstCha, oPE);

for ope=1:oPE
    for dcha=1:dstCha            
        colInd = ope + (dcha-1)*oPE;

        for scha=1:srcCha    
            for kpe=1:kPE
                for kfe=1:KernelSize(1)               
                    rowInd = kfe + (kpe-1)*KernelSize(1) + (scha-1)*KernelSize(1)*kPE;

                    Coef2(kfe, kpe, scha, dcha, ope) = Coef(rowInd, colInd);
                end
            end
        end
    end
