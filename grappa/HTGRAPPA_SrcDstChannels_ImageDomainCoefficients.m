...
function CoeffInIm = HTGRAPPA_SrcDstChannels_ImageDomainCoefficients(Coef, option, Nfe, Npe, reductionFactor)
%
...
s = size(Coef);
srcCha = floor(size(Coef,1)/option.KernelSize(1)/option.KernelSize(2));
dstCha = floor(size(Coef,2)/numel(option.OutPattern));
...
for c = 1:dstCha % for every destination coil
    ...
    currKernel = Coef(:, (c-1)*numOfMissingLines+1:c*numOfMissingLines);    
