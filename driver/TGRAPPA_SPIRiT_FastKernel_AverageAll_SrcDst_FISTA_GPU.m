...
function [unwrappedIm, fullkspace, sensitivityMap] = TGRAPPA_SPIRiT_FastKernel_AverageAll_SrcDst_FISTA_GPU(underSampledKspace, reductionFactor, option)
...
% The FISTA optimization is used
% -------------------------------------------------------------------------
...
%% compute the kernel
refMeanDst = mean(fullkspace_G, 4);
refMeanSrc = mean(fullkspace_Full_G, 4);
optionVSpirit.KernelSize = option.KernelSizeSPIRiT;
optionVSpirit.OutPattern = option.OutKernelSPIRiT;

tkernel = tic;
kernelS2D = spiritCalibration_SrcDst_MultipleOutput(refMeanSrc, refMeanDst, optionVSpirit, option.thresRegSPIRiT);
kernelD2S = spiritCalibration_SrcDst_MultipleOutput(refMeanDst, refMeanSrc, optionVSpirit, option.thresRegSPIRiT);   
% GOP_S2D = SPIRiTSrcDst(kernelS2D, unwarpMethod, [Nfe,Npe]);
% GOP_D2S = SPIRiTSrcDst(kernelD2S, unwarpMethod, [Nfe,Npe]);
...
for f=1:numOfFrame
    ...
    if ( wavWeightSPIRiT > 0 )
        ...
        lambda_factor = 0.001;
        tolerance = 1e-8;
        mMask = (reduced_k_data==0);
        Afun = @(x) GOP_S2D_D2S*(mMask.*x)-mMask.*x;
        ATfun = @(x) mMask.*(GOP_S2D_D2S'*x)-mMask.*x;

        initialKSpace = option.fullKSpaceDst(:,:,:,f);
        initialKSpace = double(initialKSpace);
        
        b = reduced_k_data-(GOP_S2D_D2S*reduced_k_data);
        
        if ( useGPU )
            [kspace, nIter, cost] = SolveFISTA_TC_GPU(Afun, ATfun, b, params.sparseTransform, params.sparseTransform', ...
                                            'initialization', initialKSpace, 'maxiteration', option.ItnlimFISTA, ... 
                                            'lambda', lambda_factor, 'tolerance', tolerance, 'ratioFISTA', option.ratioFISTA);        
        else
            [kspace, nIter, cost] = SolveFISTA_TC(Afun, ATfun, b, params.sparseTransform, params.sparseTransform', ...
                                'initialization', initialKSpace, 'maxiteration', option.ItnlimFISTA, ... 
                                'lambda', lambda_factor, 'tolerance', tolerance, 'ratioFISTA', option.ratioFISTA);
        end
    else
