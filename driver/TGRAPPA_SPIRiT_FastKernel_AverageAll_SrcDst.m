...
function [unwrappedIm, fullkspace, sensitivityMap] = TGRAPPA_SPIRiT_FastKernel_AverageAll_SrcDst(underSampledKspace, reductionFactor, option)
...
for f=1:numOfFrame
    ...
    if pocsFlagSPIRiT 
        ...
        if ( wavWeightSPIRiT > 0 )
            ...
            [kspaceLinear, RESVEC] = cgSPIRiT_SrcDst(double(reduced_k_data), [], GOP_S2D, GOP_D2S, maxIterSPIRiT, cgSPIRiTlambda, double(reduced_k_data));            
            if ( dataWeightSPIRiT > 0 )                    
                kspace = cgRegularizedSPIRiTWithLineSearch_WithoutNullSpace_SrcDst(double(reduced_k_data), GOP_S2D, GOP_D2S, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), params);
            else                
                kspace = cgRegularizedSPIRiTWithLineSearch_SrcDst(double(reduced_k_data), GOP_S2D, GOP_D2S, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), params);
            end
        else
            % perform CG optimization
            [kspace, RESVEC] = cgSPIRiT_SrcDst(double(reduced_k_data), [], GOP_S2D, GOP_D2S, maxIterSPIRiT, cgSPIRiTlambda, double(reduced_k_data));
        end
    end
