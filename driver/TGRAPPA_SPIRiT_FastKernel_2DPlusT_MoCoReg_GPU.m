...
function [unwrappedIm, fullkspace, sensitivityMap] = TGRAPPA_SPIRiT_FastKernel_2DPlusT_MoCoReg_GPU(underSampledKspace, reductionFactor, option)
        ...
        if ( ~option.performReconWithMoCo )        
            if ( dataWeightSPIRiT > 0 )
                if ( ~option.UseCoilSensitivity )
                    kspace = cgRegularizedSPIRiTWithLineSearch_2DPlusT_WithoutNullSpace_GPU(double(underSampledKspaceCurr), GOP2DT, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), params, centre, width);
                else
                    kspace = cgRegSPIRiT_2DPlusT_NoNullSpace_CoilSensitivity_GPU(double(underSampledKspaceCurr), GOP2DT, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), currS, params, centre, width);
                end
            else                
                if ( ~option.UseCoilSensitivity )
            ...
            params.mocoer = MotionCorrection2DPlusT('Linear', round(numOfFrames/2), header);        
            ...
            if ( dataWeightSPIRiT > 0 )
                if ( ~option.recoMotionCorrectedImage )
                    if ( ~option.UseCoilSensitivity )
                        kspace = cgRegSPIRiTWithLineSearch_2DPlusT_NoNullSpace_MoCoReg_GPU(double(underSampledKspaceCurr), GOP2DT, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), params, centre, width);
                    else
                        kspace = cgRegSPIRiT_2DPlusT_NoNullSpace_MoCoReg_CoilSensitivity_GPU(double(underSampledKspaceCurr), GOP2DT, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), currS, params, centre, width);
                    end
                else
                ...
                if ( ~option.UseCoilSensitivity )
                    kspace = cgRegularizedSPIRiTWithLineSearch_2DPlusT_MoCoReg_GPU(double(underSampledKspaceCurr), GOP2DT, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), params, centre, width);
                else                
                    kspace = cgRegSPIRiTWithLineSearch_2DPlusT_MoCoReg_CoilSensitivity_GPU(double(underSampledKspaceCurr), GOP2DT, min(maxIterSPIRiT, Itnlim), double(kspaceLinear), currS, params, centre, width);
                end
            end        
        end
