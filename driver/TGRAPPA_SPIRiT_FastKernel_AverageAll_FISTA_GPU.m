...
function [unwrappedIm, fullkspace, sensitivityMap] = TGRAPPA_SPIRiT_FastKernel_AverageAll_FISTA_GPU(underSampledKspace, reductionFactor, option)
% This function performs the dynamic image reconstruction using SPIRiT strategy, averaged-all kernels
% the optimization is using the FISTA
...
for f=1:numOfFrames
    ...
    reduced_k_data = double(reduced_k_data);
    refKSpace = ref(:,:,:,f);
...
%         mMask = (reduced_k_data==0);
%         Afun = @(x) GOP2*(mMask.*x);
%         ATfun = @(x) mMask.*(GOP2'*x);
%         b2 = -(GOP2*reduced_k_data);
%         [kspace, nIter, cost] = SolveFISTA_TC(Afun, ATfun, b2, params.sparseTransform, params.sparseTransform', ...
%                                         'initialization', initialKSpace, 'maxiteration', option.ItnlimFISTA, ... 
%                                         'lambda', lambda_factor, 'tolerance', tolerance, 'ratioFISTA', option.ratioFISTA);
    else
        % perform CG optimization
