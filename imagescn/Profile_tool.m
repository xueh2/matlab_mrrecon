...
%              number_distance = 0;
            ...
            h_center = plot(center_x, center_y , ...
                ['o', handlesTool.colororder(Current_Profile_index)], ...
...
for i = 1:size(update_list,1)
	if    handlesTool.Profile_info_table(update_list(i,1),update_list(i,2)).Profile_Type ~= 4
		...
		Profile_points = max(size(im))*12;
        % improfile_method='bilinear'; % or 'bilinear', 'bicubic', or 'nearest'
        improfile_method='bicubic'; % or 'bilinear', 'bicubic', or 'nearest'
        % improfile_method='nearest'; % or 'bilinear', 'bicubic', or 'nearest'
        % improfile_method='bilinear';
        % improfile_method='nearest';
        
        thick_profile_flag =0;
        if ~thick_profile_flag % normal width
            ...
            % for offset=[-2:2]
            for offset=[-5:5]
                % [Px, Py, P0(:,i,offset+3)] = improfile(im,xpts+offset*normal_vector(1),ypts+offset*normal_vector(2),Profile_points, improfile_method);
                [Px, Py, P0(:,i,offset+6)] = improfile(im,xpts+offset*normal_vector(1),ypts+offset*normal_vector(2),Profile_points, improfile_method);
            end
            P=mean(P0,3);
        end
...
% 		[Px,Py, P] = improfile(im,xpts,ypts,Profile_points, improfile_method);
        ...
        if mmode==1 % PK*
            ...
            h_row = size(handlesTool.h_all_axes, 1);
            h_col = size(handlesTool.h_all_axes, 2);
            
            im_row = floor( (update_list(i,2)-1)/h_col ) + 1;
            im_col = update_list(i,2)-1 - floor( (update_list(i,2)-1)/h_col ) * h_col + 1;
            
%             im0= getappdata(handlesTool.h_all_axes(update_list(i,2)), 'ImageData');  %DHver - get right im data
            im0= getappdata(handlesTool.h_all_axes(im_row, im_col), 'ImageData');  %DHver - get right im data
		...
		if  (xpts<0) | (xpts>size(handlesTool.Image_Data{update_list(i,2)},2)) | (ypts<0) | (ypts>size(handlesTool.Image_Data{update_list(i,2)},1))
            ...
            % generate profile for a 5*5 roi
            P = 0;
            % roiW = 2;
            roiW = 0;
            for yy=-roiW:roiW
                for xx=-roiW:roiW            
                    P = P + squeeze(handlesTool.Image_Data{update_list(i,2)}(ypts+yy, xpts+xx,:));
                end
            end
            
            P = P ./ ((2*roiW+1)*(2*roiW+1));
...
%         Nrepetitions=ceil(aspect_ratio); % repeat in time to make displayed image approx square 
        Nrepetitions=1; % fixed number of heartbeats to be repeat on time (horizontal) axis
...
h_center = plot(center_x, center_y , [profile_color,'o'], ...
	'ButtonDownFcn', 'Profile_tool(''Profile_Pos_Adjust_Entry'',1)'); 
