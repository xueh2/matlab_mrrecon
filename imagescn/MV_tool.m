...
for i = 1:numel(h_all_axes(:))
    if (ishandle(h_all_axes(i)))
...
if make_avi | make_mat
    if str2num(version('-release')) > 12.1        
        [filename, pathname] = uiputfile( {'*.*', 'Movie Files (*.m, *.avi, *.mp4)'}, ...
            'Save Movie As', 'M');
    else
        [filename, pathname] = uiputfile( {'*.*', 'Movie Files (*.m, *.avi, *.mp4)'}, ...
            'Save Movie As');
...
if make_avi
	...
	try
        ...
        elseif avicreatemethod==2
            ...
            close(vidObj);
        %elseif avicreatemethod==3 % will create black and white gif
            imap=linspace(0,1,256)';
            imap=repmat(imap,[1 3]);
            loopCount=Inf;
            for frame=1:length(M);
                if frame==1 % first image
                    [tmp] = frame2im(M(1));
                    imwrite(tmp(:,:,1),imap,[filename,'.gif'],'gif',...
                        'DelayTime',1/frame_rate, 'LoopCount',loopCount,...
                        'WriteMode','overwrite')
                else
                    [tmp] = frame2im(M(frame));
                    imwrite(tmp(:,:,1),imap,[filename,'.gif'],'gif','WriteMode','append',...
                        'DelayTime',1/frame_rate)
                end
            end            
        end
