...
%     
    ...
    try
        ...
        if(size(case_sax, 1)>0)
            process_one_sax_view_all(case_sax, resDir, series_num, pic_dir, ED, ES, dst_dir, dst_pixel_spacing, check_processed, plot_flag);
        end
...
function process_one_sax_view(pic_dir, case_prefix, ED, ES, dst_dir, gt_sax_all, gt_h_sax_all, dst_pixel_spacing, multi_slice_mode, plot_flag) 
