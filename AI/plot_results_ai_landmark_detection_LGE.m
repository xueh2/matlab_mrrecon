function plot_results_ai_landmark_detection_LGE(resDir, aiDir, pt_ids, files_record_picked, case_4chs, case_2chs, case_3chs, case_saxs, check_processed, suffix)
% plot_results_ai_landmark_detection_LGE(resDir, aiDir, pt_ids, files_record_picked, case_4chs, case_2chs, case_3chs, case_saxs, check_processed, suffix)

    pic_dir = fullfile(aiDir, 'jpg_pics', 'LGE_landmark_results');
    mkdir(pic_dir);
    ...
    for pt=1:numel(pt_ids)
        ...
        for v=1:4
            ...
            if(check_processed & exist(fullfile(contourDir, [prefix suffix '_plot.fig'])))
                continue;
            end
                        
            if(isempty(suffix))
                pts_file = fullfile(contourDir, [prefix '_AI_pts.npy']);
                probs_file = fullfile(contourDir, [prefix  '_AI_probs.npy']);
            else
            ...
            make_plots(data, pts, pic_dir, contourDir, case_used.file_names{1}, [prefix suffix], record.gt_h_view);
        end
...
function make_plots(data, pts, pic_dir, contourDir, data_file_name, prefix, gt_h_view)

    SLC = size(data, 3);

    if(SLC==1)
        h = figure;
        WC = gt_h_view.window_center;
        WW = gt_h_view.window_width;        
        imagescn(data, [WC-WW/2 WC+WW/2], [], [8]);
        hold on
        pts = pts + 1;
        if(pts(1,1)>1)
        ...
        for slc=1:SLC
            ...
            WW_set(slc, :) = [WC-WW/2 WC+WW/2];
        end
        if(mod(SLC,2)==0)
            h = figure;imagescn(data, WW_set, [2 SLC/2], [24]);
        else
            h = figure;imagescn(data, WW_set, [2 floor(SLC/2)+1], [24]);
        end
