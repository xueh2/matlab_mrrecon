...
% Last updated: November 14, 2009: Add statement "camelight"; 3-argument
%                                                             find command
...
%	Find and delete all V3D:SLICE
delete(findobj(figure_handle,'Tag','V3D:SLICE'));


handles=varargin{3};

%	Get user data associated with figure figure_handle
...
%	Find all V3D-slices and delete them
delete(findobj(figure_handle,'Tag','V3D:SLICE'));
...
if slud.alpha == 4  ||  slud.alpha == 5
    ...
    for i=1:length(temp)
        ...
        %   Test if it is an y-z slice
        if (xdata==xdata(1))
              % obere und untere Grenze bestimmen 
              % suche nach x-Wert der drüber und drunter liegt
              aug=find(ud.x<=xdata(1),1,'last');
              aog=find(ud.x>=xdata(1),1,'first');
              %aug=aug(length(aug));
              %aog=aog(1);
        ...
        %   Test if it is an x-z slice
        if (ydata==ydata(1))
              % obere und untere Grenze bestimmen 
              % suche nach y-Wert der drüber und drunter liegt
              aug=find(ud.y<=ydata(1),1,'last');
              aog=find(ud.y>=ydata(1),1,'first');
              % aug=aug(length(aug));
              % aog=aog(1);
        ...
        %   Test if it is an x-y slice
        if (zdata==zdata(1))
              % obere und untere Grenze bestimmen 
              % suche nach z-Wert der drüber und drunter liegt
              aug=find(ud.z<=zdata(1),1,'last');
              aog=find(ud.z>=zdata(1),1,'first');
              %aug=aug(length(aug));
              %aog=aog(1);
