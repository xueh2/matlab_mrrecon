...
class BallTree {
 public:
  typedef unsigned long index;              // define "index" type (long)
  const static BallTree::index NO_CHILD = (unsigned long) -1;  // indicates no further children

  /////////////////////////////
  // Constructors
  /////////////////////////////
  
  //BallTree( unsigned long d, unsigned long N, double* centers_,
  ...
  unsigned long   Npts(BallTree::index i)   const { return highest_leaf[i]-lowest_leaf[i]+1; }
  ...
  unsigned long getIndexOf(BallTree::index i) const { return permutation[i]; }
  ...
  BallTree::index closer(BallTree::index i, BallTree::index j, const BallTree& other_tree) const
      { closer(i,j,other_tree,other_tree.root()); };
 ...
 protected:
  ...
  unsigned long dims;             // dimension of data 
  unsigned long num_points;       // # of points 
  ...
  BallTree::index partition(BallTree::index, BallTree::index, BallTree::index);
