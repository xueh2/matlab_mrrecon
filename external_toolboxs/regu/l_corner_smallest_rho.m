function [reg_c,rho_c,eta_c] = l_corner_smallest_rho(rho,eta,reg_param,U,s,b,method,M)
...
% pick the first curvature maxima with smallest rho
...
if (strncmp(method,'Tikh',4) || strncmp(method,'tikh',4))
  ...
  % find all curvature maxima
  absg = abs(g);
  N = numel(g);
  
  ind = [];
  for ii=2:N-2
      if ( absg(ii)>absg(ii-1) & absg(ii)>absg(ii+1) )
          ind = [ind; ii];
      end
  end
  ...
  rho_ind = rho(ind(:));
  [mrho, mrho_ind] = min(rho_ind);
  
  if ( N > 1 )
      if ( rho_ind(mrho_ind-1)-rho_ind(mrho_ind) < 0.1 )
          mrho_ind = mrho_ind-1;
      end
  end
