...
function [mix, x, indexes] = gmminit_5classes_image_uint8(mix, imagedata, header,...
...
mix.classification = zeros(ysize, xsize, zsize, 5, 'uint8');

% mix.playing = zeros(ysize, xsize, zsize, 'uint8');
mix.playing = false(ysize, xsize, zsize);
mix.sampleInd = [];
...
mix.atlas(:,:,:,1) = uint8(round(255*(atlas_wm1+atlas_wm2)));
...
% ============================================ %
inROI = find(mix.playing==1);
[row, col, plane] = ind2sub(size(mix.playing), inROI);
clear inROI
ind1 = sub2ind(size(mix.atlas), row, col, plane, 1*ones(length(row), 1));
ind2 = sub2ind(size(mix.atlas), row, col, plane, 2*ones(length(row), 1));
ind3 = sub2ind(size(mix.atlas), row, col, plane, 3*ones(length(row), 1));
ind4 = sub2ind(size(mix.atlas), row, col, plane, 4*ones(length(row), 1));
mix.atlas(ind4(:)) = uint8(255 - mix.atlas(ind1(:)) - mix.atlas(ind2(:)) - mix.atlas(ind3(:)));
clear row col plane ind1 ind2 ind3 ind4
% ============================================ %
