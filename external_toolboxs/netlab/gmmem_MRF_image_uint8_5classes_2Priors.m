...
function [mix, options, errlog] = gmmem_MRF_image_uint8_5classes_2Priors(mix, x, options)
...
atlas = zeros(size(mix.atlas), 'uint8');
atlas = mix.atlas;
atlas(:) = 1;

MRFClassification = zeros(size(mix.classification), 'uint8');
MRFClassification = mix.classification;

MRFPriors = zeros(size(mix.priors));
MRFPriors = mix.priors;
...
%     plane = mix.indexes(:,3);
    ...
    [G, H] = ems_getMRFparams_uint8(mix.classification, mix.lkp, mix.sampleInd, 1);
    G
    H
    %[G, H] = ems_getMRFparams_uint8_noSampleInd(mix.classification, mix.lkp, row, col, plane, 1);
    % compute MRF prior
    MRFClassification(:) = ems_getMRFprior(mix.classification, mix.playing, atlas, mix.lkp, G, H);
    if ( 1 )
        mix.classification(:) = ems_getMRFprior(mix.classification, mix.playing, mix.atlas, mix.lkp, G, H);
    else
        mix.classification(:) = ems_getMRFprior(mix.classification, mix.playing, atlas, mix.lkp, G, H);
    end
%     mix = Classification2Prior_uint8(mix);
    mix = Classification2Prior_uint8_quick(mix);
    MRFPriors = Classification2Prior_uint8_quick_MRFPriors(MRFClassification, mix, MRFPriors);
