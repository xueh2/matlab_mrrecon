function [mix, options, errlog] = gmmem_MRF_image(mix, x, options)
...
for n = 1:niters
  ...
  mix = Prior2Classification(mix);
   %mix = Prior2Classification_quick(mix);
  [G, H] = ems_getMRFparams_single(mix.classification, mix.lkp, mix.sampleInd, 1);
  G
  H
  % compute MRF prior
  mix.classification(:) = ems_getMRFprior_single(mix.classification, mix.playing, mix.atlas, mix.lkp, G, H);
   mix = Classification2Prior(mix);
  %mix = Classification2Prior_quick(mix);
