...
function [mix, x, indexes] = gmminit_3classes_image_uint8(mix, imagedata, header,...
...
clear i_wm1 j_wm1 k_wm1 i_wm2 j_wm2 k_wm2
...
clear i_cortex j_cortex k_cortex
clear points
...
mix.priors = zeros(ndata, 3); % gm, wm1, wm2 classes

mix.atlas = zeros(ysize, xsize, zsize, 2, 'uint8'); % gm, wm
mix.classification = zeros(ysize, xsize, zsize, 3, 'uint8');
...
for m=1:ndata
    mix.priors(m,:) = [atlas_cortex(i(m), j(m), k(m)) atlas_wm1(i(m), j(m), k(m)) atlas_wm2(i(m), j(m), k(m)) ];
