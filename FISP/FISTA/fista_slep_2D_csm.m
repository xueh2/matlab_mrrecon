function [x, funVal,  ValueL, mse]=fista_slep_2D_csm(A, AT, y, lambda, opts)
%
%%
...
if(~isfield(opts,'pFlag'))
    opts.pFlag=1; % 1 denotes W=I, i.e., the L1
                  % 2 denotes W is the redundant Harr
end
...
if opts.pFlag==1
    ...
    proximity_operator = @(x, th) WWT(WT, soft( WW(W, x) , th ));
end


if (opts.rFlag)
    %lambda_max=max(abs(ATy),[],1);
...
%         end
    ...
    if(nargout>=2)
        ...
        if opts.pFlag==1
            funVal(iterStep)=funVal(iterStep) +lambda * sum( abs(x(:)) );
        else
            for i=1:t
                funVal(iterStep)=funVal(iterStep) +lambda * sum( abs(W(x(:,i))) );
            end
        end
