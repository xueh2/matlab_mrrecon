...
% vCSimage is of size (t x size(kcoor,1) ) x c
...
switch numel(size(coilSensitivity))
    case 3
        ...
        for i=1:sizeImage(4)
            ...
            for j=1:sizeImage(3)
                ICSimage = conj(coilSensitivity(:, :, j)) .* (nuFt' * vCSimage(index_b:index_e,j));
    ...
    case 4
        ...
        for i=1:sizeImage(4)
            ...
            for j=1:sizeImage(3)
                ICSimage = conj(coilSensitivity(:, :, j, i)) .* (nuFt' * vCSimage(index_b:index_e,j)) ...
