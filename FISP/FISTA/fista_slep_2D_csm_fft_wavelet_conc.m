...
function x=Dykstra(v,th)
    ...
    for i=1:10
        if (opts.wavelet==1)
            ...
            yy1= soft( xplusp(1: prod([2*sizeImage(1), 2*sizeImage(2), sizeImage(4)])), th);
            yy2= soft_temporal_3d ( reshape(...
                xplusp( (prod([2*sizeImage(1), 2*sizeImage(2), sizeImage(4)]) +1): end ), [2*sizeImage(1), 2*sizeImage(2), 2*sizeImage(4)]) ,th, scale);
