...
FFT_forward_t=@(x) reshape( ifftshift( fft( fftshift(...
    reshape(x, [sizeImage(1), sizeImage(2), sizeImage(4)]), 3),...
    [], 3), 3),...
    [prod([sizeImage(1), sizeImage(2), sizeImage(4)]),1] )/ sqrt( sizeImage(4) );

FFT_backward_t=@(x) reshape( ifftshift( ifft( fftshift(...
    reshape(x, [sizeImage(1), sizeImage(2), sizeImage(4)]), 3),...
    [], 3), 3),...
    [prod([sizeImage(1), sizeImage(2), sizeImage(4)]),1] ) * sqrt( sizeImage(4) );
...
function x=Dykstra(v,th)
    ...
    for i=1:10
        if (opts.wavelet==1)
            xplusp=reshape( x+p, [2*sizeImage(1), 2*sizeImage(2), sizeImage(4)]);
            yy=soft(xplusp,th); yy=yy(:);
