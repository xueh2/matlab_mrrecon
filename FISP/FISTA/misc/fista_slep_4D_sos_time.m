function [x, funVal,  ValueL, mse]=fista_slep_4D_sos_time(A, AT, y, lambda, opts)
%
%%
...
%% Problem
%
%  min  1/2 \|Ax -y\|_F^2 + lambda * sum_i \|x(:,:,i)\|_{2,1 }
%                         + 0.5*rho* sum_i \|x(:,:,i)-x(:,:,i+1)\|_F^2
%
%  x(:,:,i)=[x^i_1, x^i_2, ..., x^i_c]
%  y(:,:,i)=[y^i_1, y^i_2, ..., y^i_c] 
%
%  x and y are 3D tensors
%
%% Input parameters:
%
...
%
%%
% Composed by Jun Liu on May 09, 2011
...
%         end
    ...
    if(nargout>=2)
        ...
        for i=1:t
            funVal(iterStep)=funVal(iterStep) +lambda(i) * sum( sqrt( sum(abs(x(:,:,i)).^2,2) ) );
        end
