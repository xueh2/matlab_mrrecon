function [x, funVal,  ValueL, mse]=fista_slep_2D_csm_wavelet_Huber(A, AT, y, lambda, opts)
%
%%
...
if(isfield(opts,'sigma'))
    ...
    sigma=mean(abs(y(:))); % the parameter used in Huber's Loss
end
...
for iterStep=1:opts.maxIter
    ...
    % make some prepartion (to get w)
    As_y=As-y; abs_As_y=abs(As_y);
    w_flag=abs_As_y>sigma;
    w=ones(size(abs_As_y));
    w(w_flag)=sigma./abs_As_y(w_flag);
        
    % compute the loss at s
    r_sum0= sigma * ( sum( sum( abs_As_y(w_flag) )) - sigma/2 * sum(w_flag(:)) );
    r_sum0= r_sum0+...
        sum( sum( abs_As_y(~w_flag).^2 ))/2;
    ...
    while (1)
        ...
        % As_y here denotes Ax_y=Ax -y
        % We use this variable for saving memory
        As_y=Ax-y; abs_As_y=abs(As_y);
        w_flag=abs_As_y>sigma;
        w=ones(size(abs_As_y));
        w(w_flag)=sigma./abs_As_y(w_flag);
        
        % compute the loss at x
        l_sum0= sigma * ( sum(sum( abs_As_y(w_flag) )) - sigma/2 * sum(w_flag(:)) );
        l_sum0= l_sum0+...
            sum(sum( abs_As_y(~w_flag).^2 )) /2;
        
        % subtract the function value at s
        l_sum=l_sum0-r_sum0;
        
        % subtract the gradient term (at s)
        l_sum=l_sum- real( v' * g); 
