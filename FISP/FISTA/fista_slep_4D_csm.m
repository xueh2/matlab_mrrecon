function [x, funVal,  ValueL, mse]=fista_slep_4D_csm(A, AT, y, lambda, opts)
%
%%
% Function fista_slep_4D
%      FISTA for 4D MRA reconstruction with csm
%
%% Problem
%
...
proximity_operator = @(x, th) (soft_csm_4D(x, th));


if (opts.rFlag)
    %lambda_max=max(abs(ATy),[],1);
    lambda_max=ones(1,t)* max(abs(ATy(:))); % we use a unified lambda_max
else
    lambda_max=ones(1,t);
end
...
%         end
    ...
    if(nargout>=2)
        ...
        for i=1:t
            funVal(iterStep)=funVal(iterStep) +lambda(i) * sum( abs(x(:,i)) );
        end
