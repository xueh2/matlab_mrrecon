...
function perf_normalized = Perfusion_PrepareSignal_TwoPointNormalization(perf, perf_PD, FA_perf, FA_PD, TD, filtSize)
% perform the two-point signal normalization using PD images
% perf: [RO E1 REP SLC]
...
%     PD_fil(ind(:)) = 1;
    ...
    perf_norm_slc = 1 - perf_norm_slc;
    
    ind = find(perf_norm_slc(:)<0);
    perf_norm_slc(ind(:)) = eps;
    
    perf_norm_slc = log(perf_norm_slc);
    perf_norm_slc = perf_norm_slc * (-1/TD(s));
    
    perf_norm_slc(ind(:)) = -1;
    
    ind = find(isnan(perf_norm_slc(:)));
    perf_norm_slc(ind(:)) = -1;
