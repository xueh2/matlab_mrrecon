...
function [F, F2, F3, r, AIF_time_stamp, AIF_signal, AIF_foottime, AIF_LV_ROI, foot] = PerformQuantitativePerfusion_AIF(AIF_ori, AIF_mask, AIF, AIF_acqTime, perf, perf_mask, perf_acqTime, upsamplingRatio, numOfInternalControlPoints, lambda, numOfInternalControlPoints_L1)
% perform the quantitative perfusion computation for dual sequence
% AIF_ori : acquired AIF signal
% AIF: [RO E1 REP], normalized AIF signal
% perf: [RO E1 REP SLC]
% AIF_acqTime, perf_acqTime are in the unit of ms
% first, the AIF time stamps will be upsampled, the boundary condisiton will be border value
% second, the perfusion images will be sampled at the same time stamps as AIF images
% third, the AIF images will be processed to get LV ROI and AIF signal is computed as the averaged signal within the ROI
% the foot time of this AIF signal will be used as the starting point of devonvolution
% per-pixel perfusion impulse response will be computed, r: [RO E1 N SLC]
% per-pixel perfusion flow rate F : [RO E1 SLC], is the max(r, [], 3)
