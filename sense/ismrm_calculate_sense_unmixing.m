function [unmix, gmap] = ismrm_calculate_sense_unmixing(acc_factor, csm, snr_map, alpha)
%
...
for x=1:size(csm,1), 
    unmix(x,:,:) = ismrm_calculate_sense_unmixing_1d(acc_factor, squeeze(csm(x,:,:)), squeeze(snr_map(x,:)), alpha); 
end
