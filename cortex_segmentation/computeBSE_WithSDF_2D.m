...
function [BSE, distPercentages, distRealNumbers, dist] = computeBSE_WithSDF_2D(seg2D, segSDF, manual2D, manualSDF, xvoxelsize, yvoxelsize, distThresholds)
% compute the boundary segmentation errors with SDF input
...
segSDF(find(segSDF==0)) = max(segSDF(:));
manualSDF(find(manualSDF==0)) = max(manualSDF(:));
