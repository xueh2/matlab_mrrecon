...
if ( MRF_flag_Global )
    filename = [Global_SegResult GlobalPrefix '_segResult_4classes_MRF_uint8.hdr'];
else
    filename = [Global_SegResult GlobalPrefix '_segResult_4classes.hdr'];
end
...
% % ======================================== %
% 
% local_SegResults = LocalSegmentation_Kmeans_EM_4classes_PVs(imagedata, header, ...
%     brainmask, global_SegResult, SplittedBrain, partsNumber, tissuelabels_seg, replicate, M, saveflag, trynumber);
% 
% clear SplittedBrain
% % ====================================================== %
% 
tissuelabels_seg = [cortex wm];
saveflag = 1;
Kmeans_InitialCentres_2classes = Kmeans_InitialCentres(2:3);
% ======================================== %
k = length(tissuelabels_seg);
replicate = TryNumber;
if ( isempty(Kmeans_InitialCentres) == 0 )
    M = zeros(k,1);
    % initial intensity centroid for clustering
    M(:,1, 1) = Kmeans_InitialCentres_2classes; 
...
local_SegResults = LocalSegmentation_Kmeans_EM_2classes(imagedata, header, ...
