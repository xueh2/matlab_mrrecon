...
% centres = ClusteringSuspectedPoints_Intensity(imagedata, suspected_volume, partsNumber, TryNumber, initialCentres );

step = 4;
centres = ClusteringSuspectedPoints_Intensity(imagedata, wm_mask, partsNumber, TryNumber, initialCentres, step );

minDist = zeros(partsNumber,1);
for k = 1:partsNumber
    pointsA = centres(k, :);
    pointsB = [centres(1:k-1, :); centres(k+1:end, :)];
    [minDist(k), nearestPoints] = GetNearestPoints_VTK(pointsA, pointsB);
end
minDist
...
while(removal)
    ...
    index = find(minDist<distThres);
    if ( isempty(index) == 1 )
        removal = false
        continue;
    else
        k = index(1);
        centres = [centres(1:k-1, :); centres(k+1:end, :)];
        
        minDist = zeros(length(centres),1);
        for p = 1:length(centres)
            pointsA = centres(p, :);
            pointsB = [centres(1:p-1, :); centres(p+1:end, :)];
            [minDist(p), nearestPoints] = GetNearestPoints_VTK(pointsA, pointsB);
        end
...
filename = [Local_SegResult prefix '_SplittedBrain_Suspected_I.hdr'];
SaveAnalyze(uint32(SplittedBrain), header, filename, 'Grey' );

filename = [Local_SegResult prefix '_SplittedBrain_WM_I.hdr'];
SaveAnalyze(uint32(SplittedBrain), header, filename, 'Grey' );
...
local_SegResults = LocalSegmentation_Kmeans_EM_MRF_3classes(imagedata, header, ...
...
% neighborwidth = 3;
% 
% LabeledSeg = LabelPVs_Seg_slow_fillLabeledSeg(double(newGlobal_SegResult), header, ...
