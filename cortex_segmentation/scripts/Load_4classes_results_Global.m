...
Global_dsc_resultfile

if ( isempty(dir(Global_dsc_resultfile)) == 0 )

[Global_dsc_result, header] = LoadAnalyze(Global_dsc_resultfile, 'Grey');

if ( Tlabel == 1 )
    Global_dsc_result(find(T==0)) = 0;
end

if ( Corpus_Callosum == 1 )
    Global_dsc_result(find(Corpus>0)) = 0;
    transverseLabel(find(Corpus>0)) = 0;
    sagittalLabel(find(Corpus>0)) = 0;
    coronaryLabel(find(Corpus>0)) = 0;
end

Global_dsc_4classes = computeDSC(Global_dsc_result, classlabel, ...
...
totalNum = 0;
for tt = 1:numLabel
    index = find(Global_dsc_result==classlabel(tt));
    totalNum = totalNum + length(index);
end
tissueVolume = totalNum * voxelVolume;

savedfilename = fullfile(AssessDir, 'Global_dsc_4classes.mat');
save(savedfilename, 'Global_dsc_4classes', 'tissueVolume');
else
