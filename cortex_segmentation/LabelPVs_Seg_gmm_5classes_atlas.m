...
function [LabeledSeg, mix] = LabelPVs_Seg_gmm_5classes_atlas(mix, segResult, ...
...
%     [neighbourhood, neighbors] = GetNeighbourhood2(segResult, mix.header, i, j, k, neighborwidth);
    neighbors = GetNeighbourhood2_6neighbors_Local(segResult, mix.header, mix.offset, i, j, k, partsmask, globalSeg);
    if ( label == wmlabel )
...
%             priors(5) = nonbrainNew;
            ...
            mix.atlas(j, i, k, 1) = uint8(round(255*(priors(3)+priors(4)))); % wm
            mix.atlas(j, i, k, 2) = uint8(round(255*priors(2))); % gm
            mix.atlas(j, i, k, 3) = uint8(round(255*priors(1))); % csf
            mix.atlas(j, i, k, 4) = uint8(255 - sum(mix.atlas(j,i,k,1:3))); % non-brain
            continue;
        end
