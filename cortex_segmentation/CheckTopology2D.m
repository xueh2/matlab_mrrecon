...
for tt = 1:num
    ...
    [T_object, T_backgournd] = ComputeTopologicalNumber2D_singlePoint(i, j, B, connectivityObject, connectivityBackground);
    ...
    if ( (T_object==1) & (T_backgournd==1) )
        ...
        if ( LastData(i, j) <= 0 )
            NextData(i, j) = -10^12 * eps;
%             NextData(i, j) = LastData(i, j);
        else
            NextData(i, j) = 10^12 * eps;
%             NextData(i, j) = LastData(i, j);
        end
...
map(256, :) = [1 0 0];
map(159, :) = [0 1 0];
filename = ['./simplePoints2D/simplePoints_' num2str(ntimes) '.bmp'];
imwrite(simplePoints, map, filename, 'bmp');
