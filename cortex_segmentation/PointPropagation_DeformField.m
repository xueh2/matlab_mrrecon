...
disp(['PointPropagation_DeformField :' aviName]);
% ---------------------------------------------------------------
...
xcoord = pointStep:pointStep:W;
ycoord = pointStep:pointStep:H;

xPointNum = length(xcoord);
yPointNum = length(ycoord);

gridPts = zeros(xPointNum*yPointNum, 2);
...
for i=1:Num
    gridPts_Deformed(:, 2, i) = newH - gridPts_Deformed(:, 2, i);
end
