...
% opts.choice =1 ====> 
%              tau is set such that kept eigenvalues are larger than opts.thres * maximal eigenvalue
% opts.choice =1 ====> 
...
if (opts.choice==0)
    tau=opts.tau;
elseif (opts.choice==1)
    % set tau such that opts.percentage % energy is retained
    percentage= s./ sum(s) * 100;
    percentage = percentage(end:-1:1);
    percentage = cumsum(percentage);
    tau= min( find(percentage > opts.percentage/100) );    
else
    thres = opts.thres * s(1);
    tau= max(find(s >thres) ); 
end
...
kernel_ny=fftshift(...
    ...
    [],2), 2) * sqrt(ny);
% kernel_ny2=ifft(kernel_ny,[],2) * sqrt(ny);
% kernel_ny is of size [kkx,ny, ncnc]
...
%         sqrt(nx);
    ...
    sensitivityMap_iny=complex(zeros(nc, nx,incmax), zeros(nc, nx,incmax));
...
%         [V, D] = eig(temp'*temp);
        ...
        % different solution
        for inc = 1:incmax
%             eigD(i,i_ny,inc)=D(end-inc+1,end-inc+1) / kx/ky/ kx/ky;
            eigD(i,i_ny,inc)=D(end-inc+1,end-inc+1) / kx/ky;
            if inc==1
                sensitivityMap_iny(:, i,inc) = V(:, end-inc+1);
            elseif eigD(i,i_ny,inc)>opts.thd
                sensitivityMap_iny(:, i,inc) = V(:, end-inc+1);
            else
                sensitivityMap_iny(:, i,inc)=zeros(nc,1);
            end
            
        end
        
%         sensitivityMap_iny(:, i,2) = V(:, end-1);
%         eigD(i,i_ny,2)=D(end-1,end-1) / kx/ky;
%         
%         sensitivityMap_iny(:, i,3) = V(:, end-2);
%         eigD(i,i_ny,3)=D(end-2,end-2) / kx/ky;
        % scale by kx*ky, See Section
    end
    
    for inc = 1:incmax
        sensitivityMap(:, :, i_ny,inc)=sensitivityMap_iny(:,:,inc);
    end
%     sensitivityMap(:, :, i_ny,2)=sensitivityMap_iny(:,:,2);
%     sensitivityMap(:, :, i_ny,3)=sensitivityMap_iny(:,:,3);
    % sensitivityMap_iny of size [nc, nx]
    % sensitivityMap of size [nc, nx, ny]
end
% parallel along ny ends here

sensitivityMap=permute(sensitivityMap,[2,3,1,4]);
% permute sensitivityMap to size [nx, ny, nc]

%% normalize the output to the phase of the first coil
for ii=1:incmax
    aa=conj(sign( sensitivityMap(:,:,1,ii) ));
    sensitivityMap(:,:,:,ii)=sensitivityMap(:,:,:,ii).* repmat( aa, [1, 1, nc]);
