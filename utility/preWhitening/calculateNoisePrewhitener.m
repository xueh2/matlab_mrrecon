function  noisePrewhiteningMatrix = calculateNoisePrewhitener(noise, rx_dwelltime_data, rx_dwelltime_prescan_noise);
...
noisePrewhiteningMatrix=inv(L)*sqrt(2); % sqrt(2) used so that std(real) & std(imag) both equal 1
