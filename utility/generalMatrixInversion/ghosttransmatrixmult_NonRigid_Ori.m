function [st,gmat] = ghosttransmatrixmult_NonRigid(s,tformdata,transp_flag)
...
%      TFORMDATA.NSHOTS: number of shots
...
%       
%   Parameters for non-rigid motion correction
%      TFORMDATA.deform: array of deformation fields (dx, dy) 
%      TFORMDATA.deformInv: array of inverse deformation fields
%      TFORMDATA.keyShot: keyframe shot, (first shot is shot 1; second, short 2, etc.)
%      TFORMDATA.header: header structure to define the deformation field
%      The non-rigid warpping uses Matlab_PerformWarpingSeries2D
%
...
%-------------------------------------------------------
% the input transformation become the nonrigid deformation field
%-------------------------------------------------------
...
%           % perform the image warpping, -1: backforward
%           st = st + performNonRigidImageWarp(sa, t, tformdata, -1);
%         end
%       else
%         S = zeros(size(s));
%         for t = 1:nshots
%           %waitbar(t/nshots,hs);
%           st = performNonRigidImageWarp(s, t, tformdata, 1); % forward
%           St = fft2(st);
%           S(shots{t}) = St(shots{t});
%         end
%         st = ifft2(S);
...
%       %close(hs);
      ...
      if (nargin > 2) & strcmp(transp_flag,'transp')
        ...
        for t = 1:nshots
          %waitbar(t/nshots,hs);
          S = zeros(size(s));
          St = ifftshift(fft2(fftshift(s)));
          S(shots{t}) = St(shots{t});
          sa = ifftshift(ifft2(fftshift(S)));
          % st = st + imtransform(sa,fliptform(tforms{t}),R,'xdata',xd,'ydata',yd);     
          % perform the image warpping, -1: backforward
          st = st + performNonRigidImageWarp(sa, t, tformdata, -1);
        end
        ...
        for t = 1:nshots
          %waitbar(t/nshots,hs);
          st = performNonRigidImageWarp(s, t, tformdata, 1); % forward
          St = ifftshift(fft2(fftshift(st)));
          S(shots{t}) = St(shots{t});
        end
        st = ifftshift(ifft2(fftshift(S)));
      end
      %close(hs);
    else
      if (nargin > 2) & strcmp(transp_flag,'transp')
   ...
   % -------------------------------------------------
   % perform the non-rigid transformation
   function st = performNonRigidImageWarp(s, t, tformdata, direction)
       % direction: 1, forward warp; -1, backward warp

        % check whether it is the key shot 
        if ( t == tformdata.keyShot )
            st = s;
            return;
        end

       % perform the warpping, note s is complex image
       if ( direction == 1 ) % forward transformation
            % st = Matlab_PerformWarpingSeries2D(s, tformdata.header, tformdata.deform(t).dx, tformdata.deform(t).dy, -1);
            realPart = real(s);
            realPart = double(realPart);
            moco_real = Matlab_PerformWarpingSeries2D(realPart, tformdata.header, tformdata.deform(t).dx, tformdata.deform(t).dy, -1);

            imagPart = imag(s);
            imagPart = double(imagPart);
            moco_imag = Matlab_PerformWarpingSeries2D(imagPart, tformdata.header, tformdata.deform(t).dx, tformdata.deform(t).dy, -1);
            
            st = complex(single(moco_real), single(moco_imag));           
       else
            %st = Matlab_PerformWarpingSeries2D(s, tformdata.header, tformdata.deformInv(t).dx, tformdata.deformInv(t).dy, -1);
            realPart = real(s);
            realPart = double(realPart);
            moco_real = Matlab_PerformWarpingSeries2D(realPart, tformdata.header, tformdata.deformInv(t).dx, tformdata.deformInv(t).dy, -1);

            imagPart = imag(s);
            imagPart = double(imagPart);
            moco_imag = Matlab_PerformWarpingSeries2D(imagPart, tformdata.header, tformdata.deformInv(t).dx, tformdata.deformInv(t).dy, -1);
