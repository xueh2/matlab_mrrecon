function [res, iter, SSD] = cgPRUNO_NotLSQR_WithGT(y, PRU, nIter, lambda, x0, GT)
%
%
...
%     currSSD = GT - res;    
%     SSD(iter) = norm(currSSD(:))/sqrt(numel(GT(:)));
    
    temp = res - GT;
    SSD(iter) = sqrt(sum(abs(temp(:)).^2)/sum(abs(GT(:)).^2));
            
    if (print_residual == 1),
        fprintf('Iteration %d, norm(r) /norm(r0) = %12.8e, SSD %12.8e\n', iter, delta_new/delta_0, SSD(iter));drawnow;
    end
end
