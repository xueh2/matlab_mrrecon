function x = cgRegularizedSPIRiT(y, GOP, nIter, x0, params, centre, width)
%-----------------------------------------------------------------------
%
% x = cgRegularizedSPIRiT(y, GOP, nIter, lambda, x0)
%
...
% backtracking line-search
	...
	if (k > nIter) | (norm(dx(:))<gradToll | (prevF-f1<objToll ) )
        disp(sprintf('Final iteration : %d --- grad: %f, obj reduction: %f', k,norm(dx(:)),prevF-f1));
		break;
    end
