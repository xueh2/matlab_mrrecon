function x = cgRegularizedGRAPPAWithLineSearch(y, yGrappa, sen, nIter, x0, params, centre, width)
%-----------------------------------------------------------------------
%
% x = cgRegularizedGRAPPAWithLineSearch(y, yGrappa, nIter, ...)
%
% implementation of a regularized non linear conjugate gradient reconstruction
%
% The function solves the following problem:
%
% given k-space measurments y, and the grappa reconed kspace yGrappa
...
obj = conj(params.sen).*ifft2c(tempY-params.yGrappa);
obj = obj(:)'*obj(:);
...
gradObj = ifft2c(tempY - params.yGrappa);
gradObj = 2*fft2c(params.sen2.*gradObj);
