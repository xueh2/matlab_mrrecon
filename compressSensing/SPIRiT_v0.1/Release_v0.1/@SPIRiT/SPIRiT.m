classdef SPIRiT
    ...
    methods 
        ...
        % (c) Michael Lustig 2007
            ...
            srcCha = size(kernelOnes, 3);
            dstCha = size(kernelOnes, 4);
            
            for dst=1:dstCha
                kernelOnes(halfKfe, halfKpe, dst, dst) = 1;
            end
                        
            % kernel = kernel - kernelOnes;
            ...
            if strcmp(method,'fft')==1 | strcmp(method,'image')==1 | strcmp(method,'fft_MrFtk')==1 | strcmp(method,'fft_GPU')==1 | strcmp(method,'fft_AllGPU')==1
                for n=1:size(kernel,4)
                    ...
                    % KERNEL(:,:,:,n) = (ifft2c(zpad(kernel(end:-1:1,end:-1:1,:,n)*sqrt(imSize(1)*imSize(2)), imSize(1), imSize(2), size(kernel,3)))) - (ifft2c(zpad(kernelOnes(end:-1:1,end:-1:1,:,n)*sqrt(imSize(1)*imSize(2)), imSize(1), imSize(2), size(kernel,3))));
                end        
            ...
            res.adjoint = 0;
            res.KERNEL = KERNEL;
            res.CONJKERNEL = CONJKERNEL;

            if ( strcmp(method,'fft_GPU')==1 | strcmp(method,'fft_AllGPU')==1 )
                res.KERNELGPU = gpuArray(single(KERNEL));
                res.CONJKERNELGPU = gpuArray(single(CONJKERNEL));
            else
                res.KERNELGPU = [];
                res.CONJKERNELGPU = [];
            end   
