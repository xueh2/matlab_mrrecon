function x = cgRegularizedSPIRiTWithLineSearchAndGrappaPrior(y, GOP, sen, yGrappa, nIter, x0, params, centre, width)
%-----------------------------------------------------------------------
%
...
params.GOP = GOP;
params.y = y;
params.yGrappa = yGrappa;
params.sen = sen;
params.sen2 = sen.*conj(sen);
...
%  	f0 = objective(calibCostY, calibCostYU, calibCostDYU, x, dx, 0, params);
    ...
    if show
        ...
        h = plotKSpaceInFigure(x, [1 1 1], centre, width, 1, 1/24, h); drawnow;        
    end        
end
...
if params.grappaPriorWeight
    tempY(params.idx_nacq) = yu(params.idx_nacq) + t*dyu(params.idx_nacq);
    diffY = tempY - params.yGrappa;
    Im = ifft2c(diffY);
    grappa = conj(params.sen).*Im;
...
if params.grappaPriorWeight
    gradPrior = gradGrappaPrior(x,params);
end
...
grad = (gradObj +  params.grappaPriorWeight.*gradPrior + params.xfmWeight.*gradXFM + params.tikWeight.*gradTikhonov + params.TVWeight.*gradTV);

function gradObj = gOBJ(yu,params);
% computes the gradient of the data consistency
tempY = params.y;
tempY(params.idx_nacq) = yu(params.idx_nacq);
gradObj = params.GOP*tempY;
gradObj = ifft2c(gradObj);
gradObj = params.sen2.*gradObj;
gradObj = fft2c(gradObj);
gradObj = params.GOP'* gradObj;
gradObj = 2*gradObj;

% apply the Dc
gradObj(params.idx_acq) = 0;

function grad = gradGrappaPrior(yu,params)
tempY = params.y;
tempY(params.idx_nacq) = yu(params.idx_nacq);
tempY = tempY - params.yGrappa;
grad = params.sen2.*ifft2c(tempY);
grad = 2*fft2c(grad);
% apply the Dc
