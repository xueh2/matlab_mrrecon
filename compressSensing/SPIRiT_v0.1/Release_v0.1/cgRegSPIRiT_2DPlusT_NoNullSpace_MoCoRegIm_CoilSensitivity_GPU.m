function x = cgRegSPIRiT_2DPlusT_NoNullSpace_MoCoRegIm_CoilSensitivity_GPU(y, GOP, nIter, x0, S, params, centre, width)
%-----------------------------------------------------------------------
%
% x = cgRegSPIRiT_2DPlusT_NoNullSpace_MoCoRegIm_CoilSensitivity_GPU(y, GOP, nIter, lambda, x0)
%
...
%
% Phi(x) = ||(G-I)*F*M'*x||^2 + lambda*|W*S*x|_1 + lamda2 *||D*F*M'*x-y||_2 + ladma3 * ||x||_TV
% x is the motion corrected images [COL LIN CHA REP]
%
% the optimization method used is non linear conjugate gradient with fast&cheap backtracking
% line-search.
%
% S   - the coil sensitivity [COL LIN CHA REP]
% params.mocoer : the motion correction operator
%
% Hui Xue 2011
%-------------------------------------------------------------------------
...
idx_acq = gpuArray(single(idx_acq));
%idx_acq = single(idx_acq);

idx_nacq = find(abs(y)==0);
idx_nacq = gpuArray(single(idx_nacq));
%idx_nacq = single(idx_nacq);
...
while(1)
    ...
    if ( params.usingGPU )
        ...
        while ( lsiter<nIterSecantMax & abs(thresValue)>secantThres & (abs(thresValue)<=prevThresValue) )
            phi = wGradient(sx,params);
            phi = phi(:)'*dx(:);
            % alpha = alpha* abs(phi/(phiPrev-phi));
            alpha = alpha* phi/(phiPrev-phi);
    ...
    % if (k > nIter) | (dxNorm<gradToll) | ( (params.mocoParams.numOfIterToRefineMoCoDuringRecon==0) & (prevF-abs(f1)<objToll) )
    if (k > nIter) | (dxNorm<gradToll) | ( (~justUpdateMoCo) & (prevF-abs(f1)<objToll) )
    ...
    %% if required, refine the deformation fields
    if ( params.mocoParams.numOfIterToRefineMoCoDuringRecon > 0 )
        if ( mod(k, params.mocoParams.numOfIterToRefineMoCoDuringRecon) == 0 )
            tic
            
            params.mocoParams.sigma = params.mocoParams.sigma / params.mocoParams.sigmaDivRatio;
            
            if ( params.mocoParams.sigma < 12 )
                params.mocoParams.sigma = 12;
                params.mocoParams.numOfIterToRefineMoCoDuringRecon = 0; % reach the minimal sigma, then no need for further refinement in moco
                prevF = inf;
            end
            
            x = gather(x);            
            Im = SoS_Image_TemporalArray(x);
            r = 2048 / max(Im(:));
            ...
            clear params.mocoerOri Im
            
            disp(sprintf('Time used : %f - Iter %d - refine moction correction - %f', toc, k, params.mocoParams.sigma));
        else
            justUpdateMoCo = 0;
        end
    else
...
% motion corrected complex image
Im = yu + t.*dyu;

% go back to kspace
kspace = params.mocoer'*gather(Im);
kspace = 1/params.fftScaling*fftshift(fft2(ifftshift(kspace)));
kspace = gpuArray(single(kspace));

obj = params.GOP*kspace;
obj = single(obj);
obj = obj(:)'*obj(:);

if params.xfmWeight  
    ImCombined = sum(params.conjS.*gather(Im), 3);
    
    if ( params.usingGPU ) 
        sparseCoeff = params.sparseTransform*ImCombined;
    else
        sparseCoeff = params.sparseTransform*ImCombined;
    end
...
% F*M'*x
kspace = params.mocoer'*gather(x);
kspace = 1/params.fftScaling*fftshift(fft2(ifftshift(single(kspace))));
kspace = gpuArray(single(kspace));

% gradObj = gOBJ(x,params,kspace);

gradObj = params.GOP*kspace;
gradObj = params.GOP'*single(gradObj);
gradObj = params.fftScaling*fftshift(ifft2(ifftshift(single(gather(gradObj))))); 
gradObj = params.mocoer*double(gradObj);
gradObj = 2*gpuArray(single(gradObj));

grad = gradObj;

if params.dataWeight
    % gradDFED = gDFED(x,params,kspace);

    gradObj = kspace - params.y;
    gradObj(params.idx_nacq) = 0;

    gradObj = params.fftScaling*fftshift(ifft2(ifftshift(single(gather(gradObj))))); 
    gradObj = params.mocoer*gather(gradObj);
    gradObj = 2*gpuArray(single(gradObj));   
end
grad = (grad +  params.dataWeight.*gradObj);

clear gradDFED gradObj kspace

if params.xfmWeight
    gradXFM = gXFM(x,params);
end

grad = (grad +  params.xfmWeight.*gradXFM);
clear gradXFM

if params.TVWeight
    gradTV = gTV(x,params);
end
grad = (grad +  params.TVWeight.*gradTV);
clear gradTV

if params.tikWeight
    gradTikhonov = gTikhonov(x,params);
end
grad = (grad +  params.tikWeight.*gradTikhonov);
clear gradTV

% grad = (gradObj +  params.xfmWeight.*gradXFM + params.tikWeight.*gradTikhonov ... 
%     + params.TVWeight.*gradTV + params.dataWeight.*gradDFED);

function gradObj = gOBJ(yu,params,kspace)
% computes the gradient of the data consistency

gradObj = params.GOP'*(params.GOP*kspace);
gradObj = params.fftScaling*fftshift(ifft2(ifftshift(single(gradObj)))); 
gradObj = params.mocoer*double(gather(gradObj));
gradObj = 2*gpuArray(single(gradObj));

function gradDFED = gDFED(yu,params,kspace);
% data fidelity

gradDFED = kspace - params.y;
gradDFED(params.idx_nacq) = 0;

gradDFED = params.fftScaling*fftshift(ifft2(ifftshift(single(gradDFED)))); 
gradDFED = params.mocoer*double(gather(gradDFED));
gradDFED = 2*gpuArray(single(gradDFED));

function grad = gXFM(yu,params)
% compute gradient of the L1 transform operator
cha = size(yu, 3);
Im = sum(params.conjS.*gather(yu), 3);

if ( params.usingGPU ) 
    
    sparseCoeff = params.sparseTransform*double(Im);
